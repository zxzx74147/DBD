package com.zxzx74147.modules_dbd.auction;
...
import com.zxzx74147.modules_dbd.auction.data.AuctionData;
import com.zxzx74147.modules_dbd.info.Path;
import com.zxzx74147.modules_dbd.info.Status;
...
public class AuctionTask {
    ...
    public AuctionTask(int paimaiId,int minPrice, int price) {
        mPaimaiId = paimaiId;
        mPrice = price;
        mMinPrice = minPrice;
        doGetInfo();
    ...
    private void doGetInfo() {
        ...
        mTimer.schedule(mAuctionTask,diff-ping*2-200);
...
//        });
        ...
        request.send(new ZXHttpCallback<AuctionData>() {
            ...
            public void onResponse(ZXHttpResponse<AuctionData> response) {
                ...
                if(response.mData.getMessage().contains("成功")){
                    ...
                    int aucPrice =getPrice(price,mMinPrice,mPrice);
                    doAuction(aucPrice);
                }
    ...
    private void doFresh(final boolean doAuction){
        ...
        request.send(new ZXHttpCallback<BidListData>() {
            ...
            public void onResponse(ZXHttpResponse<BidListData> response) {
                ...
                if(doAuction){
                    int price = getPrice(mCurrentPrice,mMinPrice,mPrice);
    ...
    private static int getPrice(int currentPrice,int minPrice,int maxPrice){
        int add = (int) (Math.log(currentPrice)*Math.sqrt(currentPrice)/5);
        add=Math.max(1,add);
        add=Math.min(40,add);

        int dst=Math.max(minPrice,currentPrice+add);
        int aucPrice = Math.min(maxPrice,dst);
        return aucPrice;
    }
}
